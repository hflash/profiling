...
def AddNewNodeToSearchTree(next_node, search_tree, next_map, cost_g_next, 
...
def CalculateHeuristicCost(current_map, DG, executable_vertex, executed_vertex, 
    ...
    for leaf_node in leaf_nodes:
        ...
        for swaps in possible_swap_combination:
            ...
            for current_swap in swaps:
                ...
                if draw == True:
                    #cir_phy_next.swap(q_phy[v0], q_phy[v1])
                    if (v0, v1) in edges_DiG:
                        cir_phy_next.cx(v0, v1)
                        cir_phy_next.h(v0)
                        cir_phy_next.h(v1)
                        cir_phy_next.cx(v0, v1)
                        cir_phy_next.h(v0)
...
#@jit
def RemoteCNOTandWindowLookAhead(q_phy, cir_phy, G, DG, initial_map,
                                 shortest_length_Gs, shortest_path_G,
                                 depth_lookahead, use_prune, draw=False,
                                 DiG=None, level_lookahead=None):
