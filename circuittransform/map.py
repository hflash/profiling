...
class Map(object):
    ...
    def __init__(self, q_reg, G, initial_map = None):
        ...
        if initial_map == None:
            for q in q_reg:
                self.__DomToCod[q] = vertex[q.index]
                self.__CodToDom[vertex[q.index]] = q
...
def FindInitialMapping(DG, q_log, G, shortest_length_G):
    ...
    for node in DG.nodes():
        ...
        qubit_qubit_weight[involve_qubits[0].index][involve_qubits[1].index][1] += 1
        qubit_qubit_weight[involve_qubits[1].index][involve_qubits[0].index][1] += 1
        for q_used in involve_qubits:
            qubit_weight[q_used.index][1] += 1
...
def InitialMapSimulatedAnnealing(start_map, DG, G, DiG, q_log,
                                 shortest_length_G, shortest_path_G, 
                                 num_consider_gates=0, convergence=False):
    '''
